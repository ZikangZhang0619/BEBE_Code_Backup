...
local sample_start_date "2026-02-01"   // <-- UPDATE MANUALLY EACH WEEK
...
local sample_end_date "2026-05-18"     // <-- UPDATE MANUALLY EACH WEEK
//********************************************************************************//
...
foreach wave of local waves {
    ...
    forvalues i = 1/`n_dates' {
        ...
        local xlabels `xlabels' `dnum' `"`"`dstr'"' `"(n=`denom_i')"'"'
    }

    preserve
        keep sent_date rate_* lab_*
        reshape long rate_ lab_, i(sent_date) j(window) string
        ...
        twoway ///
            (connected rate_ sent_date if win_order == 1, sort lcolor(navy) mcolor(navy) msymbol(O) lwidth(medthin)) ///
            (scatter   label_rate sent_date if win_order == 1, mcolor(navy) msymbol(i) mlabel(lab_) mlabcolor(navy) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ sent_date if win_order == 2, sort lcolor(orange) mcolor(orange) msymbol(D) lwidth(medthin)) ///
            (scatter   label_rate sent_date if win_order == 2, mcolor(orange) msymbol(i) mlabel(lab_) mlabcolor(orange) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ sent_date if win_order == 3, sort lcolor(forest_green) mcolor(forest_green) msymbol(T) lwidth(medthin)) ///
            (scatter   label_rate sent_date if win_order == 3, mcolor(forest_green) msymbol(i) mlabel(lab_) mlabcolor(forest_green) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ sent_date if win_order == 4, sort lcolor(purple) mcolor(purple) msymbol(S) lwidth(medthin)) ///
            (scatter   label_rate sent_date if win_order == 4, mcolor(purple) msymbol(i) mlabel(lab_) mlabcolor(purple) mlabsize(tiny) mlabposition(0)), ///
