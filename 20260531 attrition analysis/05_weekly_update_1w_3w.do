...
foreach wave of local waves {
    ...
    quietly count
    if r(N) == 0 {
        di as error "No invite date has matured to the 1-week window for `wave' group comparison. Skipping this wave."
        continue
    }

    foreach win in 1w 3w {
        replace n_`win' = . if mature_`win' == 0
        gen rate_`win' = n_`win' / denom
        gen str8 lab_`win' = ""
        replace lab_`win' = string(100 * rate_`win', "%4.1f") + "%" if mature_`win' == 1
    ...
    preserve
        ...
        gen double label_rate = rate_
        replace label_rate = rate_ - 0.020 if window == "1w"
        replace label_rate = rate_ + 0.035 if window == "3w"
        replace label_rate = max(label_rate, 0.02)
        replace label_rate = min(label_rate, 1.03)

        twoway ///
            (connected rate_ x_order if group == 1 & window == "1w", sort lcolor(navy) mcolor(navy) msymbol(O) lpattern(solid) lwidth(medthin)) ///
            (scatter label_rate x_order if group == 1 & window == "1w", mcolor(navy) msymbol(i) mlabel(lab_) mlabcolor(navy) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if group == 2 & window == "1w", sort lcolor(navy) mcolor(navy) msymbol(Oh) lpattern(dash) lwidth(medthin)) ///
            (scatter label_rate x_order if group == 2 & window == "1w", mcolor(navy) msymbol(i) mlabel(lab_) mlabcolor(navy) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if group == 1 & window == "3w", sort lcolor(forest_green) mcolor(forest_green) msymbol(S) lpattern(solid) lwidth(medthin)) ///
            (scatter label_rate x_order if group == 1 & window == "3w", mcolor(forest_green) msymbol(i) mlabel(lab_) mlabcolor(forest_green) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if group == 2 & window == "3w", sort lcolor(forest_green) mcolor(forest_green) msymbol(Th) lpattern(dash) lwidth(medthin)) ///
            (scatter label_rate x_order if group == 2 & window == "3w", mcolor(forest_green) msymbol(i) mlabel(lab_) mlabcolor(forest_green) mlabsize(tiny) mlabposition(0)), ///
            ...
            xscale(range(`xmin' `xmax')) ///
            xlabel(`xlabels', angle(0) labsize(vsmall)) ///
            legend(order(1 "Control, 1 week" 3 "T1, 1 week" 5 "Control, 3 weeks" 7 "T1, 3 weeks") ///
                   rows(2) position(5) ring(0) size(vsmall) region(lcolor(none))) ///
            graphregion(color(white) margin(medium)) plotregion(color(white) margin(medium)) ///
            xsize(11.5) ysize(5.8)
...
foreach wave of local waves {
    ...
    quietly count
    if r(N) == 0 {
        di as error "No invite date has matured to the 1-week window for `wave' parent comparison. Skipping this wave."
        continue
    }
    ...
    preserve
        ...
        twoway ///
            (connected rate_ x_order if mother == 1 & window == "1w", sort lcolor(navy) mcolor(navy) msymbol(O) lpattern(solid) lwidth(medthin)) ///
            (scatter label_rate x_order if mother == 1 & window == "1w", mcolor(navy) msymbol(i) mlabel(lab_) mlabcolor(navy) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if mother == 0 & window == "1w", sort lcolor(navy) mcolor(navy) msymbol(Oh) lpattern(dash) lwidth(medthin)) ///
            (scatter label_rate x_order if mother == 0 & window == "1w", mcolor(navy) msymbol(i) mlabel(lab_) mlabcolor(navy) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if mother == 1 & window == "3w", sort lcolor(forest_green) mcolor(forest_green) msymbol(S) lpattern(solid) lwidth(medthin)) ///
            (scatter label_rate x_order if mother == 1 & window == "3w", mcolor(forest_green) msymbol(i) mlabel(lab_) mlabcolor(forest_green) mlabsize(tiny) mlabposition(0)) ///
            (connected rate_ x_order if mother == 0 & window == "3w", sort lcolor(forest_green) mcolor(forest_green) msymbol(Th) lpattern(dash) lwidth(medthin)) ///
            (scatter label_rate x_order if mother == 0 & window == "3w", mcolor(forest_green) msymbol(i) mlabel(lab_) mlabcolor(forest_green) mlabsize(tiny) mlabposition(0)), ///
            ...
            xscale(range(`xmin' `xmax')) ///
            xlabel(`xlabels', angle(0) labsize(vsmall)) ///
            legend(order(1 "Mother, 1 week" 3 "Father, 1 week" 5 "Mother, 3 weeks" 7 "Father, 3 weeks") ///
                   rows(2) position(5) ring(0) size(vsmall) region(lcolor(none))) ///
            graphregion(color(white) margin(medium)) plotregion(color(white) margin(medium)) ///
            xsize(11.5) ysize(5.8)
