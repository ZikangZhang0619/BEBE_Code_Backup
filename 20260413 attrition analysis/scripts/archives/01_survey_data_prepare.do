...
drop checkenumerator ward bed momname datestamp room_yfy room_num cutoff_date ward_num
...
drop exp_father_involv_diaper exp_father_involv_lull exp_father_involv_night exp_father_involv_play
    ...
    gen attitude_full = father_work_priority + father_strong_reliant + mothers_know_better + women_work_hurt_family + women_more_income_problem + father_time_less_valuable if mothers_know_better != .
    gen attitude_part1 = father_work_priority + father_strong_reliant if mothers_know_better == .
    gen attitude_part2 = father_work_priority + father_strong_reliant if mothers_know_better != . & father_time_less_valuable == .
...
keep father_involv_diaper father_involv_night father_involv_play father_involv_lull father_care_alone mother hospital_id survey_id
...
gen father_involv_score = father_involv_diaper + father_involv_night + father_involv_play + father_involv_lull + father_care_alone
drop father_involv_diaper father_involv_night father_involv_play father_involv_lull father_care_alone
drop survey_id
rename father_involv_score m1_father_involv_score
...
drop bed_in_room_site survey_id environment_recruitment engagement_father engagement_mother willingness_both relationship_quality mother_dominance vip
save "$proc/baseline.dta", replace
...
* Survey 643199
*---------------------*
import delimited using "$data/results-survey643199.csv", bindquote(strict) encoding("utf-8") clear
...
* Survey 714695
*---------------------*
import delimited using "$data/results-survey714695.csv", bindquote(strict) encoding("utf-8") clear
...
import delimited using "$data/results-survey795738.csv", bindquote(strict) encoding("utf-8") clear
gen survey_id = "795738"

tostring r32q0 r17q0 r97q0 r704q0, replace force
...
* Survey 448999 
*---------------------*
// only use updated predetermined characteristic at 448999
import delimited using "$data/results-survey448999.csv", bindquote(strict) encoding("utf-8") clear
...
* Survey 162992 
*---------------------*
// only use updated predetermined characteristic at 162992
import delimited using "$data/results-survey162992.csv", bindquote(strict) encoding("utf-8") clear
...
* Survey 753661 
*---------------------*
// only use updated predetermined characteristic at 753661
import delimited using "$data/results-survey753661.csv", bindquote(strict) encoding("utf-8") clear
...
* Survey 669218 
*---------------------*
// only use updated predetermined characteristic at 669218
import delimited using "$data/results-survey669218.csv", bindquote(strict) encoding("utf-8") clear
gen survey_id = "669218"
append using `1', force
append using `2m_2', force
append using `2m_3', force
append using `2m_4', force
append using `2m_5', force
append using `2m_6', force
...
gen mother = (attribute_7 == "Y")
gen hospital_id = substr(attribute_11, 4, 6) if strlen(attribute_11) > 6
keep father_involv_diaper father_involv_night father_involv_play father_involv_lull father_care_alone mother hospital_id
...
rename father_involv_score m2_father_involv_score

save "$proc/2m.dta", replace
...
* Survey 975966
*---------------------*
import delimited using "$data/results-survey975966.csv", bindquote(strict) encoding("utf-8") clear
drop if submitdate == ""
...
* Survey 721796
*---------------------*
import delimited using "$data/results-survey721796.csv", bindquote(strict) encoding("utf-8") clear

drop if submitdate == ""
gen survey_id = "721796"

append using `6m_part1'
duplicates drop firstname, force
tempfile 2
save `2'


import delimited using "$data/results-survey455394.csv", bindquote(strict) encoding("utf-8") clear
drop if attribute_11 == ""
drop if submitdate == ""
...
merge 1:1 firstname using `2', force
...
* Survey 948966
*---------------------*
import delimited using "$data/results-survey948966.csv", bindquote(strict) encoding("utf-8") clear
gen survey_id = "948966"

/* tempfile 6m_part3
save `6m_part3' */

append using `6m_part1', force
append using `6m_part2', force
...
gen mother = (attribute_7 == "Y")
gen hospital_id = substr(attribute_11, 4, 6) if strlen(attribute_11) > 6
keep father_involv_diaper father_involv_night father_involv_play father_involv_lull father_involv_feed father_care_alone mother hospital_id
...
gen father_involv_score = father_involv_diaper + father_involv_night + father_involv_play + father_involv_lull + father_involv_feed + father_care_alone
drop father_involv_diaper father_involv_night father_involv_play father_involv_lull father_involv_feed father_care_alone

rename father_involv_score m6_father_involv_score


save "$proc/6m.dta", replace
